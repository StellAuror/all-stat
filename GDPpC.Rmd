...
dfGDP |>
  ...
  scale_color_manual(values = c("#ac3524", "#cccccc")) + 
  facet_wrap(~region)
```
...
dfGDP |> 
  ...
  labs(
    x = "",
    y = "GDP per Capita (1960-2022)",
    color = "Subregion",
    title = "European GDP per Capita",
    subtitle = glue::glue("
      divided by 4 european subregions \n
...
```{r}
s <- svgstring(height = 8, width = 9, scaling = 1)
dfGDP |> 
  mutate(date = as.numeric(date)) |>
  filter(
    region %in% "Europe & Central Asia",
