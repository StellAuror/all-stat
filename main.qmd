...
data %>%
  ...
  group_by(geo, prod_nrg) %>%
  mutate(year = if_else(max(TIME_PERIOD, na.rm = T) == TIME_PERIOD, "recent", NA)) %>%
  mutate(
    order = sum(values, na.rm = T),
    prod_nrg = factor(
      prod_nrg,
      levels = c(
        # "Nuclear non-renewable natural energy inputs",
  ...
  scale_fill_manual(
    values = c(
    "Biomass based renewable natural energy inputs" = "#00b28d",
    "Hydro based renewable natural energy inputs" = "#0086f0",
    "Wind based renewable natural energy inputs" = "#8fcae9",
    "Solar based renewable natural energy inputs" = "#fff437",
    "Other renewable natural energy inputs" = "#cacaca"
    )
  )
```
...
data %>%
  ...
  ungroup() %>% group_by(geo) %>%
  mutate(
    order = sum(values, na.rm = T),
    share = if_else(prod_nrg == "Energy products", values / sum(values, na.rm = T), NA)
  ) %>%
  ggplot(
    aes(
      y = reorder(geo, order),
