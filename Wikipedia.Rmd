...
```
## The FIFA ranking {.tabset .tabset-pills}


### Position {.tabset .tabset-pills}
...
### Clear boxplot {.tabset .tabset-pills}
...
## Top Strikers {.tabset .tabset-pills}
...
### Legends od polish football {.tabset .tabset-pills}
...
### Efficiency {.tabset .tabset-pills}
...
dfTeam %>%
  mutate(isTop = player %in% mostEfficient) %>%
  ggplot(aes(
    x = goals,
    y = matches,
    size = ratio,
...
### Score distribution {.tabset .tabset-pills}
...
dfRival %>%
  ...
  geom_violin() +
  geom_point(aes(
    y =  sample(400:600, nrow(dfRival), T) / 1000
  )) 
...
dfRival %>%
  mutate(
    ...
    lLim = (q1 - (IQR * 1.5)),
    rLim = (q3 + (IQR * 1.5)),
    outlier = if_else( lLim >= RG | rLim <= RG, T, F)
  ) %>%
  ggplot(aes(
    x = RG,
    y = reorder(Rywal, RG),
...
dfRival %>%
  ...
  geom_segment(aes(
    x = GZ,
    xend = GS,
    y = reorder(Rywal, diff),
    yend = reorder(Rywal, diff)
  )) +
  geom_point(aes(
    x = GZ,
    y = reorder(Rywal, diff)
